    ...
    m_ui(new Ui::WaypointEditableView)
{
    ...
    m_ui->comboBox_action->addItem(tr("NAV: Dive"),MAV_CMD_NAV_TAKEOFF);
    ...
    m_ui->comboBox_action->addItem(tr("NAV: Ascend"),MAV_CMD_NAV_LAND);
    ...
    // add frames
    m_ui->comboBox_frame->addItem("Global/Abs. Depth",MAV_FRAME_GLOBAL);
    m_ui->comboBox_frame->addItem("Global/Rel. Depth", MAV_FRAME_GLOBAL_RELATIVE_ALT);
...
void WaypointEditableView::updateValues()
{
    ...
    emit param8Broadcast(wp->getParam8());
...
void WaypointEditableView::changedParam8(int value)
{
    wp->setParam8(value);
}
